...
import com.example.LooMotion.tool.Calc;
...
//    private int x = 0;     //X coordinate counter (Unit: m)
//    private int y = 0;     //Y coordinate counter (Unit: m)
    private static final int us = 300;
    ...
    /**
     * Method for moving with feedback
     * Befor moving detect whether there is enough space
     *
     * @param x the x coordination to the origin point
     * @param y the y coordination to the origin point
     */
    public void moveTo(float x, float y, float expectDist) {
        if (expectDist < us) return;
     ...
     */
    public void moveTo(float x, float y, float angle, float expectDist) {
        if (expectDist < us) return;
        base.addCheckPoint(x, y, angle);
    }
    ...
    private DataSlot thetaSlot = new DataSlot(8);

    // Param for portion loop
    private final float THETA_KP = Constant.THETA_KP;
    private final float THETA_KP_DISABLE_THRE = Constant.THETA_KP_DISABLE_THRE; // unit: radian

    // Param for integral loop
    private final float THETA_KI = Constant.THETA_KI;
    private final int THETA_TI = Constant.THETA_TI; // unit: *50 ms

    private final float MAX_ANGULAR_P_SPEED = Constant.MAX_ANGULAR_P_SPEED;
    private final float MAX_ANGULAR_I_SPEED = Constant.MAX_ANGULAR_I_SPEED;

    // Param for Terminate Condition
    private final float ANGULAR_ARR_DELTA = Constant.ANGULAR_ARR_DELTA;
    private final float ANGULAR_ARR_VARIANCE = Constant.ANGULAR_ARR_VARIANCE;
...
//        }
    ...
    // Variable for data processing
    private DataSlot distSlot = new DataSlot(5);
    private DataSlot pSpeedSlot = new DataSlot(2);


    // Param for portion loop
    public final float DIST_KP = 0.6f;
    public final float MAX_LINEAR_P_SPEED = 10.0f;
    public final float MIN_LINEAR_P_SPEED = 0.35f;

    // Param for integral loop
    public final float DIST_KD = 1.0f;
    public final float MAX_LINEAR_D_SPEED = 10.0f;

    public final float LINEAR_BRAKE_THRE = 1.0f;
    public final float LINEAR_ARR_DELT = 0.01f;
    public final float LINEAR_ARR_VARIANCE = 0.001f;
